$(document).ready(function () {
    ...
    $("#area-slider").slider({
        ...
        slide: function (event, ui) {
            ...
            setProgress(this, (ui.value - 30) / 120); // Оновлюємо заливку слайдера
            calculate();
        }
    });
...
// Додайте подію на введення з клавіатури для areaInput
uiStorage.areaInput.on('input', function () {
    var newValue = parseInt(uiStorage.areaInput.val());
    handleInputValue(newValue);
});

// Додайте подію на фокус для areaInput
uiStorage.areaInput.on('focus', function () {
    // Збережіть поточне значення для порівняння після введення
    uiStorage.previousValue = uiStorage.areaInput.val();
});

// Додайте подію на втрату фокусу для areaInput
uiStorage.areaInput.on('blur', function () {
    var newValue = parseInt(uiStorage.areaInput.val());
    handleInputValue(newValue);
    uiStorage.previousValue = undefined; // Очистіть попереднє значення
});

// Додайте подію на зміну слайдера
$("#area-slider").on("slide", function (event, ui) {
    uiStorage.areaInput.val(ui.value); // Оновлюємо значення areaInput при руху слайдера
    uiStorage.areaOutput.html(ui.value); // Оновлюємо значення areaOutput при руху слайдера
    calculate();
});

// Додайте обробник подій для введення з клавіатури
function handleInputValue(newValue) {
    if (!isNaN(newValue)) {
        // Обмеження значення в межах від 30 до 150
        newValue = Math.min(150, Math.max(30, newValue));

        $("#area-slider").slider("value", newValue); // Змінюємо значення слайдера при введенні
        uiStorage.areaInput.val(newValue); // Коригуємо значення areaInput
        uiStorage.areaOutput.html(newValue); // Оновлюємо значення areaOutput
        setProgress($("#area-slider")[0], (newValue - 30) / 120); // Оновлюємо заливку слайдера
        calculate();
    } else {
        // Якщо введено нечислове значення, встановлюємо попереднє значення або мінімальне
        uiStorage.areaInput.val(uiStorage.previousValue || 30);
    }
}
...
$("#area-slider").slider({
    ...
    slide: function (event, ui) {
        uiStorage.areaInput.val(ui.value); // Оновлюємо значення areaInput при руху слайдера
        uiStorage.areaOutput.html(ui.value);
        setProgress($("#area-slider")[0], (ui.value - 30) / 120); // Оновлюємо заливку слайдера
        calculate();
    }
});
