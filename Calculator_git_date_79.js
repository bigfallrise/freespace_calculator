    ...
    var calculate = function () {
        ...
        var baseProjectDate = (
            ...
            (uiStorage.fullProject.is(":checked") ? 0 : (
                ...
                (uiStorage.drafts.is(":checked") ? Math.min(1, (Math.max(0.5, ((pricearea * 0.033333) * (uiStorage.drafts.is(":checked") ? 0.3 : 0.0)))) * (uiStorage.drafts.is(":checked") ? 1 : 0.0)) : 0) +
                Math.min(0.25, (pricearea * 0.033333) * (uiStorage.estimate.is(":checked") ? 0.1 : 0.0))
            ))
        ) * areaStyleMultiplier;
        
        var projectDate = (
            (uiStorage.fullProject.is(":checked") ? 0 : baseProjectDate)
        );
        
        // Optionally, you can round the projectDate to a certain number of decimal places
        projectDate = Math.round(projectDate * 10) / 10;
