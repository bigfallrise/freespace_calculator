$(document).ready(function () {
    ...
    uiStorage.areaInput.on('input', function () {
        ...
        // Перевірка на межі мінімуму та максимуму
        if (!isNaN(newValue)) {
            // Обмеження значення в межах мінімуму та максимуму
            newValue = Math.min(Math.max(newValue, 30), 150);

            // Встановлення виправленого значення
            $("#area-slider").slider("value", newValue);
            uiStorage.areaOutput.html(newValue);
            setProgress($("#area-slider")[0], (newValue - 30) / 120);
            calculate();
        }
    });
