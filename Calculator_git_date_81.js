$(document).ready(function () {
    ...
    var calculate = function () {
        ...
        var projectDate = (
            ...
            + (uiStorage.drafts.is(":checked") ? Math.min(1, (Math.max(0.5, ((pricearea * 0.033333) * (uiStorage.drafts.is(":checked") ? 0.3 : 0.0)))) * (uiStorage.drafts.is(":checked") ? 1 : 0.0)) 
            + Math.min(0,25, pricearea* 0.033333) * (uiStorage.drafts.is(":checked") ? (uiStorage.estimate.is(":checked") ? 0.1 : 0.0) : 0))))
        ) * areaStyleMultiplier;
