    ...
    $(document).ready(function () {
        ...
        var calculate = function () {
            ...
            var projectRevenue = ((defaults.revenue +
                ...
                * replanning
                + Number.parseInt(uiStorage.areaOutput.html())
                * furnitureSelection
                + ((Math.max.apply(null, [400,(Number.parseInt(uiStorage.areaOutput.html()) * visualization)]))
                + ((Math.max.apply(null, [400,(Number.parseInt(uiStorage.areaOutput.html()) * drafts)]))
                + Number.parseInt(uiStorage.areaOutput.html())
                * estimate
        ...
        $("input[name='Building'][value='new building']").attr("checked", true);
            ...
            uiStorage.fullProject.change(function () {
                // Встановлюємо значення всіх чекбоксів, крім supervision, відповідно до стану fullProject
                var isChecked = uiStorage.fullProject.is(":checked");
                uiStorage.measurements.prop("checked", isChecked);
                uiStorage.replanning.prop("checked", isChecked);
                uiStorage.furnitureSelection.prop("checked", isChecked);
                uiStorage.visualization.prop("checked", isChecked);
                uiStorage.drafts.prop("checked", isChecked);
                uiStorage.estimate.prop("checked", isChecked);
        
                // Викликаємо клік на чекбоксах, які були перевірені
                if (isChecked) {
                    uiStorage.measurements.click();
                    uiStorage.replanning.click();
                    uiStorage.furnitureSelection.click();
                    uiStorage.visualization.click();
                    uiStorage.drafts.click();
                    uiStorage.estimate.click();
                }
        
                calculate(); // Викликаємо функцію розрахунку
            });
